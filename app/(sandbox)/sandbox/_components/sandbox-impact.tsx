...
export function SandboxImpact({ visitedSections, exploredPathways, taskStates, documentedActions, patientCheckIns, dayIndex, dayLog, workedCasesCount, onReset }: {
  ...
  return (
    <div className="min-w-0 space-y-7 [overflow-wrap:anywhere]" data-testid="sandbox-impact">
      <SectionHeading eyebrow="Program and adoption view" title="Impact, evidence, and release boundaries" description="This view demonstrates how HEARTLAND measures workflow adoption without turning sandbox activity into clinical or financial efficacy claims." action={<Button variant="outline" className="h-auto min-h-11 max-w-full flex-wrap whitespace-normal py-2" onClick={onReset}><TimerReset className="mr-2 size-4" aria-hidden="true" /> Reset sandbox</Button>} />
      ...
      <section className="rounded-3xl bg-slate-950 p-4 text-white sm:p-8">
        <div className="grid min-w-0 gap-7 lg:grid-cols-[minmax(0,1fr)_minmax(0,320px)] lg:items-center">
          ...
          <div className="min-w-0 rounded-2xl bg-white/10 p-4 sm:p-5"><div className="h-3 overflow-hidden rounded-full bg-white/15" aria-hidden="true"><div className="h-full rounded-full bg-blue-400 transition-all motion-reduce:transition-none" style={{ width: `${progress}%` }} /></div><p className="mt-3 text-xs text-slate-300">{completedTourSteps} of {tourSteps.length} activation behaviors completed</p></div>
        </div>
      </section>

      <section className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,14rem),1fr))] gap-3">
        ...
        <MetricCard label="Cases documented" value={workedCasesCount} detail="Review-queue cases with a selected synthetic outcome; a call is not required." tone="emerald" />
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)]">
        <div className="min-w-0 rounded-2xl border bg-white p-4 sm:p-6">
          <div className="flex flex-wrap items-center gap-3"><Target className="size-6 shrink-0 text-blue-700" aria-hidden="true" /><div className="min-w-0"><h2 className="text-xl font-bold text-slate-950">Activation checklist</h2><p className="text-sm text-slate-600">Value behaviors, not login vanity.</p></div></div>
          <ul className="mt-5 space-y-3">{tourSteps.map((step) => <li key={step.label} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 text-sm font-medium text-slate-700">{step.done ? <CheckCircle2 className="size-5 shrink-0 text-emerald-700" aria-hidden="true" /> : <Circle className="size-5 shrink-0 text-slate-300" aria-hidden="true" />}<span className="min-w-0">{step.label}</span></li>)}</ul>
        </div>

        <div className="min-w-0 rounded-2xl border bg-white p-4 sm:p-6">
          <div className="flex flex-wrap items-center gap-3"><BarChart3 className="size-6 shrink-0 text-violet-700" aria-hidden="true" /><div className="min-w-0"><h2 className="text-xl font-bold text-slate-950">Program report preview</h2><p className="text-sm text-slate-600">Aggregate operational measures with explicit evidence boundaries.</p></div></div>
          ...
          <Button variant="outline" className="mt-4 h-auto min-h-11 max-w-full flex-wrap whitespace-normal py-2" disabled><Download className="mr-2 size-4" aria-hidden="true" /> Synthetic report preview only</Button>
        </div>
      </section>

      <section className="rounded-2xl border border-amber-300 bg-amber-50 p-4 sm:p-6">
        <div className="flex flex-wrap gap-3"><ShieldCheck className="mt-0.5 size-6 shrink-0 text-amber-800" aria-hidden="true" /><div className="min-w-0 flex-1 basis-48"><h2 className="text-xl font-bold text-amber-950">What this sandbox cannot prove</h2><p className="mt-2 text-sm leading-6 text-amber-900">It demonstrates product capability and workflow logic. It does not prove reduced hospitalization, clinical efficacy, reimbursement, HIPAA readiness, or safe unsupervised use. Those claims require independent validation, governance, contracts, pilot evidence, and security assurance.</p></div></div>
      </section>

      <section className="rounded-3xl border border-blue-200 bg-blue-50 p-4 sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-3"><Link href="/request-access" className={cn(buttonVariants(), 'h-auto min-h-11 max-w-full flex-wrap whitespace-normal py-2')}>Request clinical workspace <ArrowRight className="ml-2 size-4" aria-hidden="true" /></Link><Link href="/guide" className={cn(buttonVariants({ variant: 'outline' }), 'h-auto min-h-11 max-w-full whitespace-normal py-2')}>Read implementation guide</Link></div>
      </section>
    </div>
  );
}
