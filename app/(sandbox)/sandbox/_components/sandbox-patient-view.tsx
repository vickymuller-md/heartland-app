...
export function SandboxPatientView({ patient, patientCheckIns, onCheckIn }: {
  ...
  return (
    <div className="min-w-0 space-y-7 [overflow-wrap:anywhere]" data-testid="sandbox-patient-view">
      ...
      <div className="grid gap-7 xl:grid-cols-[minmax(0,0.7fr)_minmax(0,1.3fr)]">
        <section className="mx-auto w-full max-w-md rounded-[2rem] border-8 border-slate-900 bg-slate-50 shadow-xl" aria-label="Synthetic mobile patient portal">
          <div className="rounded-t-[1.45rem] bg-slate-950 px-3 py-4 sm:px-5 text-white"><p className="text-xs font-semibold uppercase tracking-wide text-blue-300">HEARTLAND Today</p><h2 className="mt-1 text-xl font-bold">Good morning, {patient.name.split(' ')[0]}</h2></div>
          <div className="space-y-4 p-2 sm:p-5">
            <div className="rounded-2xl border border-blue-200 bg-blue-50 p-3 sm:p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase text-blue-700">Today’s progress</p><p className="mt-1 text-2xl font-bold text-slate-950">{completed}/{tasks.length}</p></div><div className="flex min-h-12 min-w-12 shrink-0 items-center justify-center rounded-full bg-white text-lg font-bold text-blue-700">{Math.round((completed / tasks.length) * 100)}%</div></div></div>
            ...
            {activeExperience !== 'daily-call' && (
              ...
              >
                <span className="flex size-10 shrink-0 animate-pulse motion-reduce:animate-none items-center justify-center rounded-full bg-emerald-700 text-white"><PhoneIncoming className="size-5" /></span>
                <span className="min-w-0 flex-1 basis-28"><strong className="block text-sm text-slate-950">Incoming check-in call (simulated)</strong><span className="mt-0.5 block text-xs leading-5 text-slate-600">Answer the automated daily call and play the synthetic patient</span></span>
              </button>
            )}
            ...
            {activeExperience !== 'titration-call' && (
              ...
              >
                ...
                <span className="min-w-0 flex-1 basis-28"><strong className="block text-sm text-slate-950">Titration follow-up call (simulated)</strong><span className="mt-0.5 block text-xs leading-5 text-slate-600">Answer the follow-up about the recent dose adjustment; registered safety gates decide the outcome</span></span>
              </button>
            )}
            ...
            <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 sm:p-4"><ShieldAlert className="size-5 text-amber-800" /><p className="mt-2 text-sm font-bold text-amber-950">Symptoms getting worse?</p><p className="mt-1 text-xs leading-5 text-amber-900">Follow your care team’s instructions. For a medical emergency, call emergency services.</p></div>
          </div>
        </section>
        ...
        <div className="space-y-5">
          <section className="rounded-2xl border bg-white p-3 sm:p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-3"><Button variant="outline" className="min-h-11 h-auto min-w-0 max-w-full whitespace-normal text-sm [overflow-wrap:anywhere]" onClick={() => onCheckIn(`${patient.id}-message`)}><MessageSquareText className="mr-2 size-4" /> Message care team</Button><Button variant="outline" className="min-h-11 h-auto min-w-0 max-w-full whitespace-normal text-sm [overflow-wrap:anywhere]" onClick={() => onCheckIn(`${patient.id}-call`)}><PhoneCall className="mr-2 size-4" /> View contact plan</Button></div>
          </section>
          ...
          <section className="rounded-2xl border bg-white p-3 sm:p-6">
            ...
            <ul className="mt-4 space-y-3">{patient.access.map((access) => <li key={access} className="flex gap-3 rounded-xl bg-slate-50 p-3 sm:p-4 text-sm text-slate-700"><Check className="mt-0.5 size-4 shrink-0 text-emerald-700" />{access}</li>)}</ul>
          ...
          <section className="rounded-2xl bg-slate-950 p-3 text-white sm:p-6">
            <h2 className="text-xl font-bold">Why this matters for adoption</h2>
            <div className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(min(100%,10rem),1fr))] gap-3">{[['One screen', 'Clear daily work'], ['Plain language', 'Lower cognitive burden'], ['Closed loop', 'Next action stays visible']].map(([value, label]) => <div key={value} className="rounded-xl bg-white/10 p-3 sm:p-4"><p className="font-bold">{value}</p><p className="mt-1 text-xs text-slate-300">{label}</p></div>)}</div>
          </section>
        </div>
      </div>
    </div>
