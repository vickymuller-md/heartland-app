...
export function SandboxPathways({ exploredPathways, onExplore }: {
  ...
  return (
    <div className="min-w-0 space-y-7 [overflow-wrap:anywhere]" data-testid="sandbox-pathways">
      ...
      <section className="rounded-2xl border bg-white p-3 sm:p-6">
        ...
        <ol className="mt-5 grid grid-cols-[repeat(auto-fit,minmax(min(100%,9rem),1fr))] gap-3">
          {['Signal coalesced', 'Source verified', 'Pathway reviewed', 'Action documented', 'Follow-up returns'].map((step, index) => <li key={step} className="rounded-xl bg-slate-50 p-3 sm:p-4 text-sm"><span className="text-xs font-bold text-blue-700">0{index + 1}</span><p className="mt-2 font-semibold text-slate-900">{step}</p></li>)}
        </ol>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2" aria-label="Interactive HEARTLAND pathways">
        {SANDBOX_PATHWAYS.map((pathway) => {
          ...
          return (
            <article key={pathway.id} className="flex flex-col rounded-2xl border bg-white p-3 sm:p-5 shadow-sm">
              ...
              <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 p-3 sm:p-4 text-sm text-blue-950"><strong>In this scenario:</strong> {pathway.scenarioUse}</div>
              ...
              <div className="mt-auto flex flex-wrap items-center gap-3 pt-5">
                <Link href={pathway.href} className={cn(buttonVariants(), 'h-auto min-h-11 min-w-0 max-w-full whitespace-normal py-2 [overflow-wrap:anywhere]')} onClick={() => onExplore(pathway.id)}>Open interactive tool <ArrowUpRight className="ml-2 size-4" /></Link>
                <button type="button" onClick={() => onExplore(pathway.id)} className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-950 h-auto min-w-0 max-w-full whitespace-normal [overflow-wrap:anywhere]">
      ...
      <section className="rounded-2xl border border-amber-300 bg-amber-50 p-3 sm:p-5">
      ...
      <div className="rounded-2xl border bg-slate-950 p-3 text-white sm:p-6">
