...
export function SandboxCoordination({ taskStates, onReassign, onDocumentAction }: {
  ...
  return (
    <div className="min-w-0 space-y-7 [overflow-wrap:anywhere]" data-testid="sandbox-coordination">
      ...
      <section className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,12rem),1fr))] gap-3">
      ...
      <section className="rounded-2xl border bg-white p-3 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          {SANDBOX_TEAM.map((member) => <article key={member.id} className="rounded-xl border p-3 sm:p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold text-slate-950">{member.name}</h3><p className="text-xs text-slate-500">{member.role}</p></div><span className="rounded-full bg-blue-100 px-2 py-1 text-xs font-bold text-blue-800">{member.workload} items</span></div><p className="mt-3 text-sm text-slate-700">{member.coverage}</p><p className={member.overdue ? 'mt-3 text-xs font-semibold text-amber-800' : 'mt-3 text-xs font-semibold text-emerald-700'}>{member.overdue} overdue</p></article>)}
        </div>
        ...
        <div className="mt-5 rounded-xl bg-slate-50 p-3 sm:p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">{SANDBOX_TEAM.map((member) => <Button className="h-auto min-h-11 min-w-0 max-w-full whitespace-normal px-3 py-2 text-sm [overflow-wrap:anywhere]" key={member.id} size="sm" variant={currentOwner === member.name ? 'secondary' : 'outline'} disabled={currentOwner === member.name} onClick={() => onReassign(assignableTask.id, member.name)}>Assign to {member.name.split(',')[0]}</Button>)}</div>
        </div>
      </section>

      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)]">
        <div className="rounded-2xl border bg-white p-3 sm:p-6">
          ...
          <ol className="mt-5 space-y-3">{SANDBOX_DISCHARGE_STEPS.map((step, index) => <li key={step.label} className="grid gap-3 rounded-xl border p-3 sm:p-4 sm:grid-cols-[minmax(0,0.7fr)_minmax(0,1fr)_minmax(0,2fr)] sm:items-center"><div className="flex items-center gap-2"><span className="flex size-7 items-center justify-center rounded-full bg-violet-100 text-xs font-bold text-violet-800">{index + 1}</span><strong className="text-sm">{step.label}</strong></div><span className={step.state === 'Complete' ? 'text-xs font-semibold text-emerald-700' : step.state === 'Due today' ? 'text-xs font-semibold text-amber-800' : 'text-xs font-semibold text-blue-700'}>{step.state}</span><p className="text-sm text-slate-600">{step.detail}</p></li>)}</ol>
          <Button className="mt-4 min-h-11 h-auto min-w-0 max-w-full whitespace-normal text-sm [overflow-wrap:anywhere]" variant="outline" onClick={() => onDocumentAction('James Walker: Day 14 follow-up confirmed')}>Confirm next milestone</Button>
        </div>
        ...
        <div className="rounded-2xl border bg-white p-3 sm:p-6">
          ...
          <Button className="mt-4 min-h-11 h-auto min-w-0 max-w-full whitespace-normal text-sm [overflow-wrap:anywhere]" onClick={() => onDocumentAction('Maria Santos: synthetic SBAR handoff generated')}><FileText className="mr-2 size-4" /> Generate synthetic handoff</Button>
        </div>
      </section>
      ...
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 sm:p-4 text-sm text-emerald-950"><ShieldCheck className="mr-2 inline size-4" /><strong>Governance:</strong> reassignment, access review, delivery status, and work outcomes remain auditable in the controlled workspace.</div>
    </div>
  );
}

function HandoffRow({ term, detail }: { term: string; detail: string }) {
  return <div className="rounded-xl bg-slate-50 p-3 sm:p-4"><dt className="font-bold text-slate-950">{term}</dt><dd className="mt-1 leading-6 text-slate-600">{detail}</dd></div>;
}

function OperationalCard({ icon, title, lines }: { icon: React.ReactNode; title: string; lines: string[] }) {
  return <article className="rounded-2xl border bg-white p-3 sm:p-5"><div className="flex items-center gap-2">{icon}<h2 className="font-bold text-slate-950">{title}</h2></div><ul className="mt-4 space-y-3">{lines.map((line) => <li key={line} className="flex items-start gap-2 text-sm text-slate-600"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-emerald-700" />{line}</li>)}</ul></article>;
}
