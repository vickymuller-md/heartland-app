...
export function SandboxPatientWorkspace({ patient, documentedActions, onPatientChange, onDocumentAction }: {
  ...
  return (
    <div className="min-w-0 space-y-7 [overflow-wrap:anywhere]" data-testid="sandbox-patient-360">
      ...
      <section className="rounded-2xl border bg-white p-3 sm:p-4" aria-label="Choose a synthetic patient">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          {SANDBOX_PATIENTS.map((candidate) => (
            <button key={candidate.id} type="button" aria-pressed={candidate.id === patient.id} onClick={() => onPatientChange(candidate.id)} className={`h-auto min-w-0 max-w-full whitespace-normal [overflow-wrap:anywhere] min-h-16 rounded-xl border p-3 text-left transition ${candidate.id === patient.id ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-100' : 'border-slate-200 hover:border-blue-300'}`}>
      ...
      <section className="rounded-2xl border bg-white p-3 sm:p-6">
        ...
        <div className="mt-5 rounded-xl border border-red-200 bg-red-50 p-3 sm:p-4">
      ...
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-5" aria-labelledby="brief-heading">
      ...
      <section className="rounded-2xl border bg-white p-3 sm:p-5" aria-labelledby="action-center-heading">
        ...
        {documentedActions.length > 0 && <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 sm:p-4 text-sm text-emerald-950"><ShieldCheck className="mr-2 inline size-4" /><strong>Latest simulated action:</strong> {documentedActions.at(-1)}</div>}
      </section>

      <Tabs defaultValue="monitoring" className="rounded-2xl border bg-white p-3 sm:p-5">
        <TabsList className="h-auto min-h-12 w-full flex-wrap justify-start" aria-label="Synthetic patient record sections">
          <TabsTrigger className="h-auto min-h-11 min-w-0 max-w-full whitespace-normal px-3 py-2 text-sm [overflow-wrap:anywhere]" value="monitoring">Monitoring</TabsTrigger>
          <TabsTrigger className="h-auto min-h-11 min-w-0 max-w-full whitespace-normal px-3 py-2 text-sm [overflow-wrap:anywhere]" value="medications">Medications & labs</TabsTrigger>
          <TabsTrigger className="h-auto min-h-11 min-w-0 max-w-full whitespace-normal px-3 py-2 text-sm [overflow-wrap:anywhere]" value="timeline">Timeline</TabsTrigger>
          <TabsTrigger className="h-auto min-h-11 min-w-0 max-w-full whitespace-normal px-3 py-2 text-sm [overflow-wrap:anywhere]" value="plan">Plan & access</TabsTrigger>
        </TabsList>

        <TabsContent value="monitoring" className="mt-5">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.75fr)]">
            <div className="rounded-xl border p-3 sm:p-4"><h3 className="font-bold text-slate-950">Weight trend</h3><p className="mt-1 text-xs text-slate-500">Each point exposes source timing; no recommendation is generated.</p><WeightTrend data={patient.vitals} /></div>
          ...
          <div role="region" aria-label="Synthetic vital history, horizontally scrollable" tabIndex={0} className="mt-5 max-w-full overflow-x-auto rounded-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600"><table className="w-full min-w-[620px] text-left text-sm"><caption className="sr-only">Synthetic vital history</caption><thead><tr className="border-b text-xs uppercase text-slate-500"><th className="p-2">When</th><th className="p-2">Weight</th><th className="p-2">SBP</th><th className="p-2">Heart rate</th><th className="p-2">SpO₂</th></tr></thead><tbody>{patient.vitals.map((vital) => <tr key={vital.label} className="border-b last:border-0"><td className="p-2 font-medium">{vital.label}</td><td className="p-2">{vital.weight} lb</td><td className="p-2">{vital.sbp}</td><td className="p-2">{vital.heartRate}</td><td className="p-2">{vital.spo2}%</td></tr>)}</tbody></table></div>
        </TabsContent>

        <TabsContent value="medications" className="mt-5">
          <div className="grid gap-5 lg:grid-cols-2">
            <div><h3 className="flex items-center gap-2 font-bold text-slate-950"><Pill className="size-4" /> Medication reconciliation</h3><div className="mt-3 space-y-3">{patient.medications.map((med) => <div key={med.name} className="rounded-xl border p-3 sm:p-4"><div className="flex flex-wrap justify-between gap-2"><strong>{med.name}</strong><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold">{med.status}</span></div><p className="mt-1 text-sm text-slate-700">{med.therapyClass} · {med.dose}</p><p className="mt-2 text-xs text-slate-500">{med.note}</p></div>)}</div></div>
            <div><h3 className="flex items-center gap-2 font-bold text-slate-950"><FlaskConical className="size-4" /> Labs and freshness</h3><div className="mt-3 space-y-3">{patient.labs.map((lab) => <div key={lab.name} className="rounded-xl border p-3 sm:p-4"><div className="flex flex-wrap items-center justify-between gap-2"><strong>{lab.name}</strong><span className={lab.status === 'within range' ? 'text-xs font-semibold text-emerald-700' : 'text-xs font-semibold text-amber-700'}>{lab.status}</span></div><p className="mt-1 text-lg font-bold text-slate-950">{lab.value}</p><p className="text-xs text-slate-500">Collected {lab.collected}</p></div>)}</div></div>
          </div>
        </TabsContent>

        <TabsContent value="timeline" className="mt-5">
          <ol className="space-y-3">{patient.timeline.map((event) => <li key={event.id} className="grid gap-2 rounded-xl border p-3 sm:p-4 sm:grid-cols-[120px_1fr]"><p className="text-xs font-semibold text-slate-500">{event.when}</p><div><p className="font-bold text-slate-950">{event.title}</p><p className="mt-1 text-sm text-slate-600">{event.detail}</p>{event.outcome && <p className="mt-2 text-xs font-semibold text-emerald-700">Outcome: {event.outcome}</p>}</div></li>)}</ol>
        </TabsContent>

        <TabsContent value="plan" className="mt-5">
          <div className="grid gap-5 lg:grid-cols-3">
            <div className="rounded-xl border p-3 sm:p-4"><h3 className="font-bold text-slate-950">Care plan</h3><ol className="mt-3 space-y-3">{patient.carePlan.map((item, index) => <li key={item} className="flex gap-3 text-sm text-slate-700"><span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-bold text-blue-800">{index + 1}</span>{item}</li>)}</ol></div>
            <div className="rounded-xl border p-3 sm:p-4"><h3 className="font-bold text-slate-950">Education</h3><p className="mt-3 text-3xl font-bold text-slate-950">{patient.education.completed}/{patient.education.total}</p><p className="text-sm text-slate-600">modules complete</p><p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900"><strong>Next:</strong> {patient.education.next}</p></div>
            <div className="rounded-xl border p-3 sm:p-4"><h3 className="font-bold text-slate-950">Access & privacy</h3><ul className="mt-3 space-y-3">{patient.access.map((item) => <li key={item} className="flex gap-2 text-sm text-slate-700"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-emerald-700" />{item}</li>)}</ul></div>
          </div>
        </TabsContent>
      </Tabs>

      <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 sm:p-4 text-sm text-amber-950"><AlertTriangle className="mr-2 inline size-4" /><strong>Boundary:</strong> the brief organizes source information; it does not independently diagnose, prescribe, or replace record review and professional judgment.</div>
    </div>
  );
}

function BriefCard({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return <div className="rounded-xl border bg-white p-3 sm:p-4"><div className="flex items-center gap-2 text-sm font-semibold text-slate-800">{icon}{title}</div><div className="mt-2 space-y-1 text-sm text-slate-700">{children}</div></div>;
}

function ActionButton({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return <Button type="button" variant="outline" className="min-h-14 justify-start text-left h-auto min-w-0 max-w-full whitespace-normal text-sm [overflow-wrap:anywhere]" onClick={onClick}>{icon}<span className="ml-2">{label}</span></Button>;
}
